...
class NorBlock(nn.Module): #nor块定义
...
class NorNet(nn.Module): #残差网络结构定义
    ...
    def forward(self,x):
        ...
        out = self.fc(out)
        return out
def PlainNet18():#系列NorNet实现 #去掉sohrtcut
    return NorNet(NorBlock,[2,2,2,2])
def PlainNet34():
    return NorNet(NorBlock,[3,4,6,3])