...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, class_num=10, use_se=False, se_reduction=16,dropout=0.0):
        super().__init__()
        self.use_se = use_se
    ...
    def forward(self, x):
        ...
        out = self.dropout(out) #0.3概率让输入特征置零
        out = self.fc(out)
        return out
...
def SEResNetDrop18(reduction=16,dropout=0.3):
    return ResNet(ResBlock, [2, 2, 2, 2], use_se=True, se_reduction=reduction,dropout=dropout)

#def SEResNetDrop34(reduction=16):
    #return ResNet(ResBlock, [3, 4, 6, 3], use_se=True, se_reduction=reduction,dropout)